...
class CCTcpip(CChannel):
    ...
    def _cc_send(self, msg: bytes or str, **kwargs) -> None:
        ...
        """
        if not self.socket:
            raise TypeError("Channel must be opened before messages can be sent")
    ...
    def _cc_receive(self, timeout=0.01) -> Dict[str, Optional[bytes]]:
        ...
        """
        if not self.socket:
            raise TypeError("Channel must be opened before messages can be received")
        ...
        try:
            ...
            log.exception(f"encountered timeout error while receiving message via {self}")
            return {"msg": None}
        except Exception:
